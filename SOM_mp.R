...
#Set the w directory
setwd("D:/GoogleDrive/Programming/R/Visual/Assignment")
#get the wd
getwd()
...
###  LOAD DATA  ### ----
# load SIMD data
data <- read.csv(file="SIMD_Edimburgh2.csv", sep=";", header = TRUE)

# load normalized ranking data for Edinburgh
rank_edi <- read.csv(file="SIMD_2016_edi_ranks.csv", sep=",", header = TRUE)
...
# save the image
ggsave(paste("images/Node_counts_train112",".jpeg", sep = ""), 
...
# save the image
ggsave(paste("images/Node_quality_train",".jpeg", sep = ""), 
       plot(som_model_train, type = "quality", shape="straight", main="Node Quality/Distance", palette.name=coolBlueHotRed), 
       width = 19.4, height = 12.7, units = c("cm"), dpi = 500)
...
# save the image
ggsave(paste("images/cluster",".jpeg", sep = ""), 
       plot(som_model_fin, type="mapping", bgcol = clus_pal[som_model_fin_clust],
            main = "Clusters", labels="", heatkey="FALSE", shape="straight"), 
       width = 19.4, height = 12.7, units = c("cm"), dpi = 500)
...
# save the image
ggsave(paste("images/cluster_clus",".jpeg", sep = ""), 
       plot(som_model_fin, type="mapping", bgcol = clus_pal[som_model_fin_clust],
            main = "Clusters", labels="", heatkey="FALSE", shape="straight") +
            add.cluster.boundaries(som_model_fin, som_model_fin_clust),
       width = 19.4, height = 12.7, units = c("cm"), dpi = 500)
...
# save the image
ggsave(paste("images/piechart2",".jpeg", sep = ""), 
       plot(som_model_fin, type="codes",codeRendering = "segments", shape="straight",
            main = "SOM custer details", palette.name=coolBlueHotRed),
...
# save the image
ggsave(paste("images/piechart_clus2",".jpeg", sep = ""), 
       plot(som_model_fin, type="codes",codeRendering = "segments", shape="straight",
            main = "SOM neighbour distances", palette.name=coolBlueHotRed) +
         add.cluster.boundaries(som_model_fin, som_model_fin_clust),
       width = 19.4, height = 21, units = c("cm"), dpi = 500)
...
# save the image
ggsave(paste("images/geography_of_clusters2",".png", sep = ""), 
       p, 
...
write.csv(som_stats2, file = "SomStats.csv",row.names=FALSE)
...
write.csv(som_stats_red, file = "som_stats_red.csv",row.names=FALSE)
...
# Get in R a Pivot table done on the file SomStats.csv I exported before
clust_prof <- read.csv(file="PivotGraph.csv", sep=",", header = TRUE)
...
ggsave(paste("images/cluster_profile2",".jpeg", sep = ""), plot = scatter, width = 19.4, height = 12.7, units = c("cm"), dpi = 500)
